...
public class GroupBuyingEvaluationRecyclerAdapter extends RecyclerView.Adapter {

    private FragmentActivity activity;
    private List<ShopEvaluationBean.ListsBean> lists;

    public GroupBuyingEvaluationRecyclerAdapter(FragmentActivity activity, List<ShopEvaluationBean.ListsBean> lists) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        GroupBuyingEvaluationGridAdapter groupBuyingEvaluationGridAdapter =
                new GroupBuyingEvaluationGridAdapter(activity, null);
        holder1.gridView.setAdapter(groupBuyingEvaluationGridAdapter);

        //是否匿名(0-不匿名 1-匿名)
        if (lists.get(position).getIs_anon().equals("0")) {
            //评价内容图像，展示位圆形
            Glide.with(activity).load(lists.get(position).getFile_path()).asBitmap().centerCrop().into(new BitmapImageViewTarget(holder1.evaluationImg) {
                @Override
