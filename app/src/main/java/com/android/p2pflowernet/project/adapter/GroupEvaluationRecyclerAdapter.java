...
public class GroupEvaluationRecyclerAdapter extends RecyclerView.Adapter {
    ...
    public GroupEvaluationRecyclerAdapter(FragmentActivity activity, List<GroupEvaluationBean.EvalListBean> lists) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        List<String> img_list = lists.get(position).getImg_list();
//        给用来显示图片的gridview设置适配器
        GroupEvaluationGridAdapter groupBuyingEvaluationGridAdapter = new GroupEvaluationGridAdapter(img_list);
        holder1.gridView.setAdapter(groupBuyingEvaluationGridAdapter);
        //是否匿名(0-不匿名 1-匿名)
        if (lists.get(position).getIs_anon().equals("0")){
            //评价内容图像，展示位圆形
            Glide.with(activity).load(lists.get(position).getHeader_img()).asBitmap().centerCrop().placeholder(R.mipmap.default_header).into(new BitmapImageViewTarget(holder1.evaluationImg) {
                @Override
