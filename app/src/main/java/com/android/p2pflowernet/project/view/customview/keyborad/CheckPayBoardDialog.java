 ...
 * description:
 */
public class CheckPayBoardDialog implements XNumberKeyboardView.IOnKeyboardListener {
    ...
    public CheckPayBoardDialog(Activity activity, int tag) {
    ...
    public CheckPayBoardDialog builder() {
        if (mTag == 0) {
            // 只弹出键盘
...
//        dialog.setCancelable(false);
    ...
    // 点击返回
    public CheckPayBoardDialog setBackKey(final View.OnClickListener listener) {
    ...
    private void setLayout() {
        ...
        mCheckpaypwd.setOnTextFinishListener(new PayPwdEditText.OnTextFinishListener() {
            ...
            public void onFinish(String str) {
                ...
                EventBus.getDefault().post(new CheckPwdEvent(str));

                // 检验输入的支付密码
                tempTextstr = str;
                Prompt_tv.setText("输入完成");
                Prompt_tv.setTextColor(getResources().getColor(R.color.black));
                mCheckpaypwd.clearText();
                if (mTag == 0) {
                    if (onPayStatusListener != null)
                        onPayStatusListener.onPayStatus("取消", true);
                }
            }
        });
    }
