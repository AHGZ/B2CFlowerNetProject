...
public class SuspendButtonLayout extends RelativeLayout implements View.OnTouchListener {
    ...
    private OnClickLitener onLClickliTener;

    public interface OnClickLitener {

        void onLCickLitener(View view);
    }

    public void setOnLClickliTener(OnClickLitener onLClickliTener) {
        this.onLClickliTener = onLClickliTener;
    }
    ...
    public SuspendButtonLayout(Context context, AttributeSet attrs) {
        ...
        // 获取属性
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.SuspendButtonLayout, 0, 0);
        try {
            ...
            // 按钮的大小，默认小于主按钮与子按钮间距
            mImageSize = ta.getDimension(R.styleable.SuspendButtonLayout_imageSize, mDistance / 2);

            if (mImageSize > mDistance) mImageSize = mDistance;

            // 按钮图片资源
            mResMainClose = ta.getResourceId(R.styleable.SuspendButtonLayout_imageMainClose, R.drawable.dsj_probe);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (Math.abs(lastX - downX) < 1 && Math.abs(lastY - downY) < 1) {
                    ...
                    if (onLClickliTener != null) {

                        onLClickliTener.onLCickLitener(imageMain);
                    }
//                    openSuspendButton();
//                    closeSuspendButton();
                }
    ...
    public void setMainOpenImageResource(int res) {
        mResMainOpen = res;
        if (suspendedStatus == SUSPEND_BUTTON_OPENED || suspendedStatus == SUSPEND_BUTTON_OPENING) {
            imageMain.setImageResource(mResMainOpen);
        }
    }
