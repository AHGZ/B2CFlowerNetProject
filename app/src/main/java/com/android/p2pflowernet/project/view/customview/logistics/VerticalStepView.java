package com.android.p2pflowernet.project.view.customview.logistics;
...
public class VerticalStepView extends LinearLayout implements VerticalStepViewIndicator.OnDrawIndicatorListener {
    ...
    private List<String> mTextTimes;
    private int mComplectingPosition;
    private int mUnComplectedTextColor = ContextCompat.getColor(getContext(), R.color.uncompleted_text_color);//定义默认未完成文字的颜色;
    private int mComplectedTextColor = ContextCompat.getColor(getContext(), R.color.text_complter);//定义默认完成文字的颜色;
    ...
    public VerticalStepView setStepViewTextTimes(List<String> texts) {
        mTextTimes = texts;
        return this;
    }
...
//                        mTextView.setTypeface(null, Typeface.BOLD);
                    ...
                    timeTextView.setGravity(Gravity.BOTTOM);
                    timeTextView.setLayoutParams(layoutParams1);
                    timeTextView.setTextColor(Color.parseColor("#4B4B4B"));
                    timeTextView.setText(mTextTimes.get(i));
                    mTextContainer.addView(timeTextView);

                    //设置分割线
                    View view = new View(getContext());
                    LinearLayout.LayoutParams layoutParams3 = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, 1);
